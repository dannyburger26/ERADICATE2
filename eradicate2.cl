...
void eradicate2_score_leading(const uchar * const hash, __global result * const pResult, __global const mode * const pMode, const uchar scoreMax, const uint deviceIndex, const uint round) {
	...
	uint calculatedScore = 0;
	uint leadingZeroCount = 0;
	uint leadingFourCount = 0;
	bool breakFlag = false;

	// First Loop: Process leading zeroes and special cases
	for (uint i = 0; i < 20; i++) {
		uint a = hash[i] & 0xF0;
		uint b = hash[i] & 0x0F;
	...
	// Second Loop: Continue processing from `leadingZeroCount`
	if (!breakFlag) {
		for (uint i = leadingZeroCount + 1; i < 20; i++) {
			if (hash[i] == 0x44) {
				leadingFourCount += 2;
				continue;
			}
	...
	// Third Loop: General scoring for all bytes
	for (uint i = 0; i < 20; i++) {
		uint a = hash[i] & 0xF0;
		uint b = hash[i] & 0x0F;
	...
	eradicate2_result_update(hash, pResult, calculatedScore, scoreMax, deviceIndex, round);
}
