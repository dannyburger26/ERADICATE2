...
std::string makePreprocessorInitHashExpression(const std::string & strAddressBinary, const std::string & strInitCodeDigest, const std::string & strWalletAddress) {
...
int main(int argc, char * * argv) {
	try {
		...
		argp.addSwitch('X', "wallet", strWallet);
		argp.addSwitch('I', "init-code-hash", strInitCode);
		...
		const std::string strInitCodeDigest = parseHexadecimalBytes(strInitCode);
		const std::string strWalletAddress = parseHexadecimalBytes(strWallet);
		const std::string strPreprocessorInitStructure = makePreprocessorInitHashExpression(strAddressBinary, strInitCodeDigest, strWalletAddress);
